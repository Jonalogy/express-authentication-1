...
passport.use(new LocalStrategy({
  ...
  db.user.find({
    ...
    // valid password is what we defined in user model methods, passing it to the bcrypt
    if (!user || !user.validPassword(password)) {
      // null says there is no error, but either user name not found or password not valid
      cb(null, false);
    } else {
      // null says there is no error, and user is found, password is valid too
      cb(null, user);
    }
    // there is an error, passing the error through catch
  }).catch(cb);
}));
